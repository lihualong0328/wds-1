...
//                  (BSWP<<1);
    ...
    case MODE_TFT_16BIT_240320:
        ...
        LCDCON4 = HSPW_240320;
        LCDCON5 = (FORMAT8BPP_565<<11) | (HSYNC_INV<<9) | (VSYNC_INV<<8) | \
                  (HWSWP<<1);   /* 图中, VSYNC是高电平有效，有些lcd要求低电平有效或都行。HSYNC_INV：信号是否反转 */

        /*
		* 把 frame buffer地址告诉 2440
		*/
        /*
         * 设置LCD控制器的地址寄存器LCDSADDR1~3
         * 帧内存与视口(view point)完全吻合，
...
void Lcd_Palette8Bit_Init(void)
{
	...
	LCDCON1 &= ~0x01;	// stop lcd controller，不同于书上
    
    LCDCON5 |= (FORMAT8BPP_565<<11); // 设置调色板中数据格式为5:6:5

    palette = (volatile unsigned int *)PALETTE; // 设置调色板首地址，PALETTE不是sdram地址，是lcd控制器内的256*2B
    for (i = 0; i < 256; i++)
 ...
 *               1 - LCD_PWREN输出无效
 */
void Lcd_PowerEnable(int invpwren, int pwren)   /* 设置LCD_PWREN，可忽略 */
{
    //GPGCON = (GPGCON & (~(3<<8))) | (3<<8);   // GPG4用作LCD_PWREN，jz2440 没用
...
void Lcd_EnvidOnOff(int onoff)
{
    if (onoff == 1)
    {
        LCDCON1 |= 1;         // ENVID ON，即能输出信号
		GPBDAT |= (1<<0);			// Power on，打开背光
    }
    else
    {
...
{
    TPAL = (1<<24)|((color & 0xffffff)<<0);// 使能 | 颜色值
}
